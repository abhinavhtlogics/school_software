...
class FeeAmountController extends Controller		
{
	...
	{ 
		$fee_amounts=FeeAmount::leftJoin('course_master', 'fee_amounts.course_id', '=', 'course_master.courseId') 
					->leftJoin('class_master', 'fee_amounts.class_id', '=', 'class_master.classId') 				
					->select('fee_amounts.*', 'course_master.courseName', 'class_master.className')		
					->get();	

        if(!empty($fee_amounts)) {   
			return response()->json(["status" => 'successed',"data" => $fee_amounts]);    	  	
		}
		else {
	...
	public function create(Request $request)			
	{
		...
		{
			$response_arr=array("status"=>"failed","message"=>'Fee Amount already exist for some class!!');	  
            return response()->json($response_arr); 
		} 
		else
		{
			...
			$inserts=FeeAmount::insert($insert_arr);  
