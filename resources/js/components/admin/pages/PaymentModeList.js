...
class PaymentModeList extends Component {
 ...
 handleDelete(id) {
		...
		if (result.value) {		//
				axios.delete(`/paymentmode/delete/${id}`)  						
				  .then((response) => {			
 ...
 getData(){
   ...
   axios.get('/paymentmode/index').then((response) => {		
  ...
  render() { 
	...
	let items = (this.state.items.length>0)?this.state.items:[];  
        ...
        itemLists = items.map((item) => {	
		  ...
		  return (
				<tr key={item.id}>
				  ...
				  <td>{item.pay_type}</td> 				      				  	
				  <td> 							
				   <a href={`/payment_mode_edit/${item.id}`} className='btn'>		
